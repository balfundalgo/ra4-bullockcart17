...
# For PyInstaller EXE, __file__ points to temp dir — use exe location instead
...
STATE_FILE = BASE_DIR / "ra4_daily_state.json"
...
INDEX_CONFIG = {
    "NIFTY": {
        ...
        "exchange": "NSE",
        "strike_gap": 50,
        "lot_size": 65,             # Default fallback (updated dynamically)
    ...
    "SENSEX": {
        ...
        "exchange": "BSE",
        "strike_gap": 100,
        "lot_size": 20,              # Default fallback (updated dynamically)
...
# ── Scrip Master for dynamic lot sizes ──
MASTER_URL = "https://images.dhan.co/api-data/api-scrip-master.csv"

def fetch_lot_sizes() -> Dict[str, int]:
    """Download Dhan scrip master CSV and extract lot sizes for indices.
    Matches ha_strategy_dhan.py build_index_option_lots() exactly."""
    import csv, io
    log.info("  Fetching lot sizes from Dhan scrip master...")
    try:
        ...
        lots = {}
        names = ["MIDCPNIFTY", "FINNIFTY", "BANKNIFTY", "NIFTYNXT50",
                 "NIFTY", "SENSEX50", "BANKEX", "SENSEX"]
        for row in rows:
        ...
        log.info(f"  Lot sizes from master: {lots}")
        return lots
    except Exception as e:
        log.error(f"  Failed to fetch scrip master: {e}")
        return {}


def update_lot_sizes():
    """Fetch lot sizes and update INDEX_CONFIG dynamically."""
    lots = fetch_lot_sizes()
    for idx_name, cfg in INDEX_CONFIG.items():
        if idx_name in lots:
            old = cfg["lot_size"]
            cfg["lot_size"] = lots[idx_name]
            if old != lots[idx_name]:
                log.info(f"  {idx_name} lot size updated: {old} → {lots[idx_name]}")
        else:
            log.warning(f"  {idx_name} lot size not found in master, using default {cfg['lot_size']}")
...
def api_post(endpoint: str, payload: dict, retries: int = 2,
    ...
    # Build headers fresh each call (matches working ha_strategy_dhan.py pattern)
    hdrs = {
        "Content-Type": "application/json",
    ...
    # Sanity check
    if not hdrs["access-token"] or not hdrs["client-id"]:
        log.error(f"  API {endpoint} ABORTED — missing credentials! "
                  f"token={'SET' if hdrs['access-token'] else 'EMPTY'}, "
                  f"client-id={'SET' if hdrs['client-id'] else 'EMPTY'}")
        return None

    # Option chain endpoints have special 1-per-3-second limit
    ...
    for attempt in range(retries + 1):
        try:
            log.info(f"  API POST {endpoint} (attempt {attempt+1}/{retries+1})")
            r = requests.post(url, headers=hdrs, json=payload, timeout=15)
            log.info(f"  → HTTP {r.status_code} | body[0:200]: {r.text[:200]}")
            if r.status_code == 200:
                try:
                    return r.json()
                except Exception as je:
                    log.error(f"  → JSON parse failed: {je}")
                    if attempt < retries:
                        time.sleep(2)
                    continue
            elif r.status_code == 429:
                wait = 2 ** (attempt + 1)
                log.error(f"  → 429 RATE LIMITED. Backoff {wait}s...")
                time.sleep(wait)
                continue
            else:
...
def fetch_expiry_list(index_name: str) -> List[str]:
    ...
    log.info(f"  Fetching expiry list: {payload}")
    # Verify credentials are set
    if not HEADERS.get("access-token"):
        log.error(f"  HEADERS missing access-token! Headers keys: {list(HEADERS.keys())}")
        return []
    if not HEADERS.get("client-id"):
        log.error(f"  HEADERS missing client-id!")
        return []
    resp = api_post("/optionchain/expirylist", payload)
    if resp and resp.get("status") == "success":
        data = resp.get("data", [])
        log.info(f"  Expiries found: {data}")
        return data
    # Log the full response so we can debug
    log.error(f"  Expiry list API failed for {index_name}. Response: {resp}")
    return []
...
def get_target_expiry(index_name: str) -> Optional[str]:
    ...
    if not expiries:
        log.error(f"  No expiries returned from API for {index_name}")
        return None

    today = now_ist().date()
    offset = INDEX_CONFIG[index_name]["expiry_offset"]
    log.info(f"  Today={today}, offset={offset}, raw expiries count={len(expiries)}")
    ...
    for exp_str in expiries:
        try:
            ...
            log.warning(f"  Could not parse expiry date: '{exp_str}'")
            continue

    expiry_dates.sort(key=lambda x: x[0])
    log.info(f"  Future expiries: {[e[1] for e in expiry_dates]}")
    ...
    log.error(f"  No future expiries found for {index_name} (all expired?)")
    return None
...
def place_order(security_id: str, segment: str, side: str,
                qty: int, price: float = 0, order_type: str = "MARKET",
                validity: str = "DAY", product: str = "INTRADAY") -> dict:
    """Place order via Dhan REST API.
    Copied from working ha_strategy_dhan.py — direct requests, fresh headers."""
    RATE_LIMITER.wait_for_order_slot()

    # Fresh headers per call (matches working script)
    hdrs = {
        "Content-Type": "application/json",
        "access-token": HEADERS.get("access-token", ""),
        "client-id": HEADERS.get("client-id", ""),
    ...
    log.info(f"  [ORDER] {side} {segment}:{security_id} qty={qty} type={order_type}")

    for att in range(3):
        try:
            r = requests.post(f"{BASE_URL}/orders", headers=hdrs, json=pl, timeout=15)
        except Exception as e:
            log.error(f"  [ORDER] Network error attempt {att+1}: {e}")
            time.sleep(1.0)
            continue

        if r.status_code == 429:
            log.warning(f"  [ORDER] 429 rate limited, attempt {att+1}")
        ...
        if r.status_code != 200:
            try:
                ...
                body = r.text[:300]
            log.error(f"  [ORDER REJECTED] HTTP {r.status_code}: {body}")
            return {"filled": False, "status": "HTTP_ERR", "order_id": ""}

        try:
            d = r.json()
        except:
            log.error(f"  [ORDER] Bad JSON response: {r.text[:200]}")
            return {"filled": False, "status": "BAD_JSON", "order_id": ""}

        status = str(d.get("orderStatus", "")).upper()
        oid = str(d.get("orderId", "") or "")
        avg_price = float(d.get("averageTradedPrice", 0) or 0)

        if status in ("REJECTED", "CANCELLED"):
            err = d.get("omsErrorDescription") or d.get("errorMessage") or "rejected"
            log.error(f"  [ORDER REJECTED] {side} {segment}:{security_id} id={oid} → {err}")
            return {"filled": False, "status": status, "order_id": oid}

        log.info(f"  [ORDER SENT] {side} {segment}:{security_id} id={oid} status={status}")

        if status == "TRADED" and avg_price > 0:
            log.info(f"  [ORDER FILLED] {side} qty={qty} @ ₹{avg_price:.2f}")
            return {"filled": True, "price": avg_price, "status": "TRADED", "order_id": oid}

        # Order accepted but not yet filled — poll for confirmation
        if oid:
            log.info(f"  [ORDER] Polling for fill confirmation...")
            for poll in range(8):
                time.sleep(0.5)
                try:
                    RATE_LIMITER.wait_for_order_slot()
                    pr = requests.get(f"{BASE_URL}/orders/{oid}", headers=hdrs, timeout=10)
                    if pr.status_code == 200:
                        pd = pr.json()
                        ps = str(pd.get("orderStatus", "")).upper()
                        pp = float(pd.get("averageTradedPrice", 0) or 0)
                        if ps == "TRADED" and pp > 0:
                            log.info(f"  [ORDER FILLED] {side} qty={qty} @ ₹{pp:.2f}")
                            return {"filled": True, "price": pp, "status": "TRADED", "order_id": oid}
                        if ps in ("REJECTED", "CANCELLED"):
                            log.error(f"  [ORDER NOT FILLED] id={oid} status={ps}")
                            return {"filled": False, "status": ps, "order_id": oid}
                except Exception as pe:
                    log.warning(f"  [ORDER] Poll error: {pe}")

            log.warning(f"  [ORDER NOT FILLED] id={oid} — timeout after polling")
            return {"filled": False, "status": "TIMEOUT", "order_id": oid}

        return d  # Fallback — return whatever Dhan sent

    log.error(f"  [ORDER] All 3 attempts failed for {side} {segment}:{security_id}")
    return {}
...
class BullockCart17Engine:
    ...
    # ─── STATE PERSISTENCE (daily) ───

    def save_state(self):
        """Save current day's state to JSON for resume after restart."""
        state = {
            "date": now_ist().strftime("%Y-%m-%d"),
            "trackers": {},
            "index_trade_count": dict(self.index_trade_count),
            "closed_trades_count": len(self.closed_trades),
            "closed_trades_pnl": sum(t.total_pnl for t in self.closed_trades),
        }
        for key, t in self.trackers.items():
            state["trackers"][key] = {
                ...
                "current_lock": t.current_lock,
                "safety_remaining": t.safety_remaining,
                "kuber_lock": t.kuber_lock,
                "kuber_active": t.kuber_active,
                "hanuman_active": t.hanuman_active,
                "hanuman_lock": t.hanuman_lock,
                "hanuman_high": t.hanuman_high,
                "tier_index": t.tier_index,
            }
        try:
            STATE_FILE.write_text(json.dumps(state, indent=2))
            log.info(f"  [STATE] Saved daily state to {STATE_FILE.name}")
        except Exception as e:
            log.error(f"  [STATE] Save failed: {e}")

    def load_state(self):
        """Load today's state from JSON if exists. Allows resuming mid-day."""
        if not STATE_FILE.exists():
            log.info("  [STATE] No previous state file found — fresh start")
            return False
        try:
            ...
            if saved_date != today:
                log.info(f"  [STATE] State file is from {saved_date}, today is {today} — fresh start")
                return False

            # Restore tracker states
            for key, data in state.get("trackers", {}).items():
                if key in self.trackers:
                    t = self.trackers[key]
                    t.total_pnl = data.get("total_pnl", 0)
                    t.trade_count = data.get("trade_count", 0)
                    t.is_stopped = data.get("is_stopped", False)
                    t.current_lock = data.get("current_lock", 0)
                    t.safety_remaining = data.get("safety_remaining", 0)
                    t.kuber_lock = data.get("kuber_lock", 0)
                    t.kuber_active = data.get("kuber_active", False)
                    t.hanuman_active = data.get("hanuman_active", False)
                    t.hanuman_lock = data.get("hanuman_lock", 0)
                    t.hanuman_high = data.get("hanuman_high", 0)
                    t.tier_index = data.get("tier_index", -1)

            # Restore index trade counts
            for idx, count in state.get("index_trade_count", {}).items():
                self.index_trade_count[idx] = count

            prev_pnl = state.get("closed_trades_pnl", 0)
            prev_count = state.get("closed_trades_count", 0)
            log.info(f"  [STATE] Resumed from today's state: {prev_count} trades, "
                     f"PnL=₹{prev_pnl:+,.0f}")

            # Emit to GUI
            self._emit("status", {"msg": f"Resumed: {prev_count} trades, PnL=₹{prev_pnl:+,.0f}"})
            return True
    ...
    def initialize(self):
        ...
        # Fetch dynamic lot sizes from Dhan scrip master
        update_lot_sizes()

        # Load previous state if resuming same day
        self.load_state()
    ...
    def _exit_lot(self, trade: ActiveTrade, lot: LotState, exit_price: float, reason: str):
        ...
        if all(not l.is_active for l in trade.lots):
            ...
            # Save state after every trade close for resume capability
            self.save_state()
    ...
    def update_config(self, new_config: StrategyConfig):
        """Update running config from GUI. Called when user clicks SAVE."""
        old = self.config
        self.config = new_config
        changes = []
        for field in [
            "paper_mode", "ema_fast", "ema_slow", "adx_period", "adx_threshold",
            "atr_period", "target_premium", "premium_range_low", "premium_range_high",
            "de1_day_target_per_index", "de1_day_sl_per_index",
            "de2_day_sl_per_index", "de2_safety_line",
            "trade_start_time", "trade_end_time", "cooldown_minutes",
            "max_sl_per_trade", "max_trades_per_day_per_index",
            "logic_vikalp_enabled", "logic_epel_enabled", "logic_exit_plus1_enabled",
            "logic_vivek_enabled", "logic_kuber_enabled",
            "logic_hanuman_tail_enabled", "logic_tiered_risk_enabled",
        ]:
        ...
        if changes:
            for c in changes:
                log.info(f"  [CONFIG] {c}")
            self._emit("config_saved", {"changes": changes})
        else:
            log.info("  [CONFIG] No changes detected")
            self._emit("config_saved", {"changes": []})

    def stop(self):
        self.stop_event.set()
        self.save_state()  # Persist state for resume
        if self.ws:
            try:
                self.ws.close()
            except Exception:
